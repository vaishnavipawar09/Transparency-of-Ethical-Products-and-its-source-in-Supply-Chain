	...
	protected void doGet(HttpServletRequest request,
		...
		try {
			...
			ResultSet rsLogin12 = stRegister12
					.executeQuery("select * from medicine_info_order where D_Id='"
							+ mid[1] + "'");
			if (rsLogin12.next()) {
			...
			if (oqty <= oldqty) {
				...
				st101.executeUpdate("update medicine_info set Qty='" + totalqty
						+ "' where MId='" + mid[0] + "'");
				Statement stRegister101 = con.createStatement();
				String acc = "1";
				stRegister101
						.executeUpdate("update medicine_info_order set Status_Info='"
								+ msg
								+ "' where D_Id='"
								+ mid[1]
								+ "' and MId='" + mid[0] + "'");
				...
				String datainfo = MedicineName + "@0@" + CompanyName + "@0@"
						...
						+ "@0@" + ExpiryDate + "@0@" + Medicine_No+"@0@"+mid[0];
				String encg =AESencrp.encrypt(datainfo);
				Create_QR.CreateQR(encg, path);
				String imagedata="D_"+MedicineName;
				 SendMail.SendImage(Distribute_Email_ID,path,imagedata);
